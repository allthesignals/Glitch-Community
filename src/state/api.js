  ...
  return useMemo(
    () => ({
      ...
      addProjectToCollection: ({ project, collection }) =>
        api.put(`/v1/collections/${collection.id}/projects/${project.id}`),
      orderProjectInCollection: ({ project, collection }, index) =>
        api.put(`/v1/collections/${collection.id}/projects/${project.id}`, { index }),
      updateProjectInCollection: ({ project, collection }, data) =>
        api.put(`/v1/collections/${collection.id}/projects/${project.id}`, data),
      removeProjectFromCollection: ({ project, collection }) =>
        api.delete(`/v1/collections/${collection.id}/projects/${project.id}`),
      ...
      updateProjectMemberAccessLevel: ({ project, user, accessLevel }) =>
        api.put(`v1/projects/${project.id}/users/${user.id}`, { accessLevel }),

      // teams
      joinTeam: ({ team }) => api.post(`/v1/teams/${team.id}/join`),
      inviteEmailToTeam: ({ team }, emailAddress) =>
        api.post(`/v1/teams/${team.id}/tokens`, { email: emailAddress }),
      ...
      updateUserAccessLevel: ({ user, team }, accessLevel) =>
        api.put(`/v1/teams/${team.id}/users/${user.id}`, { accessLevel }),
      ...
      addPinnedProject: ({ project, team, user }) =>
        api.put(`/${entityPath({ team, user })}/pinnedProjects/${project.id}`),
      removePinnedProject: ({ project, team, user }) =>
        api.delete(`/${entityPath({ team, user })}/pinnedProjects/${project.id}`),

      // Glitch PRO
      // TODO: v1/payments/glitchPro -> v1/payments/:productName
      getSubscriptionStatus: () => api.get('/v1/payments/glitchPro'),
      createSubscriptionSession: ({ successUrl, cancelUrl }) =>
        api.post('/v1/payments/glitchPro', { successUrl, cancelUrl }),
